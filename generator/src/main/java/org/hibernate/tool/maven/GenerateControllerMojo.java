...
@Mojo(name = "hbm2controller", defaultPhase = GENERATE_SOURCES)
public class GenerateControllerMojo extends AbstractGenerationMojo {
    ...
    protected void executeExporter(MetadataDescriptor metadataDescriptor) {
        Exporter pojoExporter = ExporterFactory.createExporter(NextExporterType.CONTROLLER.className());
        ...
        if (repositoryPackageName != null && repositoryPackageName.trim().length() > 0) {
            pojoExporter.getProperties().setProperty("repositoryPackageName", repositoryPackageName.trim());
        }
        if (servicePackageName != null && servicePackageName.trim().length() > 0) {
            pojoExporter.getProperties().setProperty("servicePackageName", servicePackageName.trim());
        }
        if (requestMapping != null && requestMapping.trim().length() > 0) {
            pojoExporter.getProperties().setProperty("requestMapping", requestMapping.trim());
        }
        getLog().info("Starting Controller export to directory: " + outputDirectory + "...");
        pojoExporter.start();
    }
