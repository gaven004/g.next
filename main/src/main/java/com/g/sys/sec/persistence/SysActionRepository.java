...
public interface SysActionRepository extends
    ...
    @Query("select a from SysAction a where (a.resource = ?1 or a.resource like '%*%') and (a.method = ?2 or a.method = 'ALL')")
    Iterable<SysAction> findByResourceAndMethod(String resource, ActionMethod method);
}